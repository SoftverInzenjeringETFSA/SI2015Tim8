...
public class ProizvodServisTest {
	...
	public void brisiZauzeto(){
		try{
			...
			ArrayList<Proizvod> pr= new ArrayList<Proizvod>();
			pr= proizvodS.dajSveProizvode();
			for(int i=0; i<pr.size();i++){
				proizvodS.obrisiProizvod(pr.get(i));
			}
	...
	public void testDajProizvod() {
		...
		assertEquals(proizvodM1.getNaziv(),artikal.getNaziv());
	}
	...
	public void testKreirajProizvod() {
		...
		try{			
			...
			assertEquals(proizvodM1.getNaziv(),proizvodPoredjenja.getNaziv());
	...
	public void testIzmijeniProizvod() {
		...
		proizvodIzmjena=proizvodS.dajProizvod(id3);
